...
class ResNet(nn.Module):
    ...
    def base_forward(self, x, is_list=None):
        ...
        if is_list:
            ...
            f1_tmp = self.att1(x_list[0], x)
            f2_tmp = self.att2(x_list[1], x)
            f3_tmp = self.att3(x_list[2], x)
            ...
            f1_tmp = self.att1(torch.cat((x1_list[0], x2_list[0])), torch.cat((x1, x2)))
            f2_tmp = self.att2(torch.cat((x1_list[1], x2_list[1])), torch.cat((x1, x2)))
            f3_tmp = self.att3(torch.cat((x1_list[2], x2_list[2])), torch.cat((x1, x2)))
            x = f1_tmp + f2_tmp + f3_tmp
