...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, zero_init_residual=False, groups=1,
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(832, 64, kernel_size=3, stride=1, padding=1, bias=False),
            norm_layer(64),
            nn.ReLU(inplace=True))
        ...
        self.att_blocks = [SpattialAttentionBlock(self.att_channel * 2, self.att_channel, 3, self.activation).to(self.device) for _ in range(3)]
    ...
    def base_forward(self, x, is_list=None):
        ...
        if is_list:
            ...
            for ii in range(len(x)):
                x_tmp = self.conv1(x[ii])
                x_list.append(x_tmp)
            x_s = torch.stack(x_list)
            x_sum = torch.sum(x_s, dim=0)
            x = [att_block(x, x_sum) for att_block, x in zip(self.att_blocks, x_list)]
            ...
            for ii in range(len(x1_tmp)):
                x_tmp = self.conv1(torch.cat((x1_tmp[ii], x2_tmp[ii])))
            ...
            x1 = [att_block(x, x1_sum) for att_block, x in zip(self.att_blocks, x1_list)]
            ...
            x2 = [att_block(x, x2_sum) for att_block, x in zip(self.att_blocks, x2_list)]
            x2 = torch.sum(torch.stack(x2), dim=0)
            x2_list.append(x2)
...
def _resnet(arch, block, layers, pretrained, **kwargs):
    ...
    if pretrained:
        ...
        conv2_weight = state_dict['conv2.0.weight'].cpu().data
        conv2_weight_new = conv2_weight.repeat(1, 13, 1, 1)
        state_dict['conv2.0.weight'] = nn.Parameter(conv2_weight_new)
