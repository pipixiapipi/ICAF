    ...
    def base_forward(self, x, is_list=None):
        ...
        if is_list:
            ...
            for ii in range(len(x)):
                ...
                x_list.append(x_tmp)
            x_s = torch.stack(x_list[:3])
            x_sum = torch.sum(x_s, dim=0)
            x = [att_block(x, x_sum) for att_block, x in zip(self.att_blocks, x_list[:3])]
            ...
            x1 = [att_block(x, x1_sum) for att_block, x in zip(self.att_blocks, x1_list[:3])]
            ...
            x2 = [att_block(x, x2_sum) for att_block, x in zip(self.att_blocks, x2_list[:3])]
...
def _resnet(arch, block, layers, pretrained, **kwargs):
    ...
    if pretrained:
        ...
        conv3_weight_new = conv3_weight.repeat(1, 13, 1, 1)
