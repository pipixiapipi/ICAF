    ...
    def base_forward(self, x, guide):
        if guide:
            ...
            for ii in range(len(x1)):
                # print(ii)
                x_tmp = self.relu(self.bn1(self.conv1(x1[ii])))
                x_fea_lst.append(x_tmp)
            x1 = torch.stack(x_fea_lst)
            x1 = torch.sum(x1, dim=0)

            x1 = [att_block(x_i, x1) for att_block, x_i in zip(self.att_blocks, x_fea_lst)]
            x1 = torch.sum(torch.stack(x1), dim=0)

            # self.visualize_feature_maps_heatmap(x1)
            ...
            # x0 = self.conv2(self.conv1(x0))
            #
            # for ii in range(len(x1)):
            #     x_tmp = self.conv2(self.conv1(x1[ii]))
            #     x_fea_lst.append(x_tmp)
            # x_sum = torch.stack(x_fea_lst)
            # x_sum = torch.sum(x_sum, dim=0)
            #
            # tmp0 = self.att0(x_fea_lst[0], x_sum)
            # tmp2 = self.att2(x_fea_lst[2], x_sum)
            # x = torch.cat((x_fea_lst[0], x_fea_lst[2], x_sum), dim=1)
            # x1 = self.conv2()
        else:
            x_fea_lst = []
