...
class ResNet(nn.Module):
    ...
    def base_forward(self, x, PCKA=False):
        if PCKA:
            ...
            x_l_aug, x_l, x_u_w_aug, x_u_w = x
            # x_l_aug, x_u_w_aug = self.relu(self.bn1(self.conv1(x_l_aug))), self.relu(self.bn1(self.conv1(x_u_w_aug)))
            # for i in range(3):
                # x_l[i] = self.relu(self.bn1(self.conv1(x_l[i])))
                # x_u_w[i] = self.relu(self.bn1(self.conv1(x_u_w[i])))
            x_l_aug = self.apply_conv_bn_relu(x_l_aug)
            x_u_w_aug = self.apply_conv_bn_relu(x_u_w_aug)
            x_l = [self.apply_conv_bn_relu(x_l[i]) for i in range(3)]
            x_u_w = [self.apply_conv_bn_relu(x_u_w[i]) for i in range(3)]

            f1_l = self.att1(x_l[0], x_l_aug)
            f2_l = self.att2(x_l[1], x_l_aug, f1_l)
            x_l = self.att3(x_l[2], x_l_aug, f2_l)

            f1_u_w = self.att1(x_u_w[0], x_u_w_aug)
            f2_u_w = self.att2(x_u_w[1], x_u_w_aug, f1_u_w)
            f_u_w = self.att3(x_u_w[2], x_u_w_aug, f2_u_w)

            x = torch.cat((x_l, f_u_w))
